...
def get_admin_metrics_summary(
    ...
    El rango de fecha aplica a la tabla gig_payment (created_at).
    """
    ...
    # ================================
    # ⚠️ NUEVO: Turnos completados = pagos generados
    # -------------------------------
    completed_shifts_query = select(func.count(GigPayment.id))

    if date_from is not None:
        completed_shifts_query = completed_shifts_query.where(GigPayment.created_at >= date_from)
    if date_to is not None:
        completed_shifts_query = completed_shifts_query.where(GigPayment.created_at <= date_to)

    completed_shifts = session.exec(completed_shifts_query).one()
    # ================================
    ...
    return AdminMetricsSummary(
        ...
        completed_shifts=completed_shifts or 0,  # ahora viene desde pagos
